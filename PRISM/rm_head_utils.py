...
def load_reward_head(prefer="auto", cache_path=_DEFAULT_CACHE, device="cpu"):
    ...
    prefer:
      ...
      "cache"      -> require the cached file at cache_path.
    """
    if prefer in ("auto", "cache") and os.path.exists(cache_path):
        w = torch.load(cache_path, map_location="cpu").float().reshape(1, 4096)
    ...
    if prefer == "seqclf":
        ...
        w = m.score.weight.detach().float().reshape(1, 4096)   # [1, 4096], the TRUE head
        _maybe_cache(w, cache_path)
        return w.reshape(-1, 1).to(device)

    if prefer in ("auto", "range"):
        try:
            w = _fetch_head_via_range()
            _maybe_cache(w, cache_path)
            return w.reshape(-1, 1).to(device)
    ...
    w = m.score.weight.detach().float().reshape(1, 4096)
    _maybe_cache(w, cache_path)
    return w.reshape(-1, 1).to(device)
