"""CA Experiment 2 -- the main result on Community Alignment: does the shared
population direction add anything beyond the base reward head?

Same three directions as the PRISM exp2, in the 4096-dim Skywork reward space:
  head     unit base reward head (Skywork score.weight)
  v_pop    the LoRe v2 shared population direction = unit(V @ wbar), from the committed
           CA fit, in its natural (data) orientation -- NOT re-oriented to the head
  residual unit(v_pop - (v_pop.head) head) -- the part of v_pop the head misses

For each we report concept alignment (cosine to the 11 concept vectors, and which
clear the random-direction null) and held-out pair accuracy on the CA test split.
We also report cos(v_pop, head) and the pair-score correlation, since cosine alone
understates behavioral overlap (two near-orthogonal directions can still rank pairs
similarly by projecting onto shared embedding structure).

CA is the high-data regime (~180 pairs/user vs PRISM's ~15), so if v_pop still beats
the head here, the shared direction is a real learned axis, not a low-data artifact.
...
def main():
    ...
    print(f"[ca-exp2] config={args.config} tau={tau:.4f} "
          f"{len(test_diffs)} held-out users")
    ...
    summary = {
        ...
        "resid_norm_over_vpop_norm": float((v_pop - float(v_pop @ head) * head).norm()),
        "directions": {
            k: {
    ...
    for k, r in reports.items():
        print(f"[ca-exp2] {k:9s} sig={r['n_significant']:2d} "
              f"acc={r['held_out_accuracy']['overall_pair_acc']:.4f} "
              f"cos(head)={r['cos_to_head']:+.4f}")
    print(f"[ca-exp2] cos(v_pop, head)={float(v_pop @ head):+.4f}  "
          f"corr(scores v_pop, head)={summary['score_corr_vpop_head']:+.4f}")
    print(f"[ca-exp2] wrote {out_dir}/summary.json")
